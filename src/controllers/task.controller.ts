...
export const createTask = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const task = await taskUseCase.createTask(projectId, userId, req.body);
...
export const getProjectTasks = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const result = await taskUseCase.getProjectTasks(projectId, userId, {
      status,
      priority,
      page,
      limit,
...
export const getTaskById = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const task = await taskUseCase.getTaskById(id, userId);
...
export const updateTask = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const id = req.params.id as string;
    const updatedTask = await taskUseCase.updateTask(id, userId, req.body);
...
export const deleteTask = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const id = req.params.id as string;
    await taskUseCase.deleteTask(id, userId);
    res.status(204).send();
  } catch (error) {
    next(error);
  }
